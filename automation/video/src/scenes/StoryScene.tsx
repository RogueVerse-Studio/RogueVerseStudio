...
export const StoryScene: React.FC<{headline: string; body: string; image: string}> = ({headline, body, image}) => {
  ...
  const isCharacterCutout = image.toLowerCase().endsWith('.png');
  return (
    <AbsoluteFill style={{background: 'radial-gradient(circle at 50% 30%, #43171d 0%, #101521 56%, #05070c 100%)', overflow: 'hidden'}}>
      <CanvasImage name="Scene artwork" src={staticFile(image)} style={{height: 1920, width: 1080, objectFit: isCharacterCutout ? 'contain' : 'cover', objectPosition: 'center', opacity: isCharacterCutout ? 0.84 : 0.58, scale: interpolate(frame, [0, 6 * fps], [1.04, 1.14], {extrapolateLeft: 'clamp', extrapolateRight: 'clamp', easing: Easing.linear, output: 'perceptual-scale'})}} />
